...
public class Frame {
    ...
    public Frame() {
        ...
        AccountsTab.add(btnPrintAccounts, BorderLayout.SOUTH);

        btnPrintAccounts.addActionListener(e -> printAccounts());
        ...
        JButton btnPrintJournal = new JButton("Print Journal");
        JournalTab.add(btnPrintJournal, BorderLayout.SOUTH);
         
        btnPrintJournal.addActionListener(e -> printJournal());
        ...
        JButton btnPrintLedger = new JButton("Print Selected Ledger");
        LedgerTab.add(btnPrintLedger, BorderLayout.SOUTH);

        btnPrintLedger.addActionListener(e -> printSelectedLedger());
...
//================= Printing features ==========================
    ...
    private void printBalanceSheet() {
        try {
            MessageFormat header1 = new MessageFormat("BALANCE SHEET - ASSETS");
            MessageFormat header2 = new MessageFormat("BALANCE SHEET - LIABILITIES & EQUITY");
    ...
    private void printAccounts() {
        try {
            MessageFormat header = new MessageFormat("ACCOUNTS LIST");
            MessageFormat footer = new MessageFormat("Page {0}");
    
            accountsTable.print(JTable.PrintMode.FIT_WIDTH, header, footer);
        } catch (PrinterException e) {
            JOptionPane.showMessageDialog(frame, "Printing failed: " + e.getMessage());
        }
    }  
    ...
    private void printJournal() {
        try {
            MessageFormat header = new MessageFormat("GENERAL JOURNAL");
            MessageFormat footer = new MessageFormat("Page {0}");
   
            journalTable.print(JTable.PrintMode.FIT_WIDTH, header, footer);
        } catch (PrinterException e) {
            JOptionPane.showMessageDialog(frame, "Printing failed: " + e.getMessage());
        }
    }
    //print ledger
    private void printSelectedLedger() {
        try {
            String selectedAccount = ledgerAccountDropdown.getSelectedItem().toString();
   
            if (selectedAccount == null || selectedAccount.isEmpty()) {
                JOptionPane.showMessageDialog(frame, "No account selected.");
                return;
            }
   
            MessageFormat header = new MessageFormat("GENERAL LEDGER - " + selectedAccount);
            MessageFormat footer = new MessageFormat("Page {0}");
